                ...
                {items.map((item) => {
                    return (
                        <li key={item.title} className="mb-10 ms-6">
                            <span className="absolute flex items-center justify-center w-7 h-7 rounded-full -start-[0.90rem] bg-pink-red mt-1"></span>
                            <item.type title={item.title} company={item.company} date={item.date} empType={item.empType}>
                                {item.body}
                            </item.type>
                        </li>
                    );
...
export function TimeLineItem(props: TimeLineItemProps) {
    return (
        <div className="w-full">
            <h3 className="flex items-center mb-1 text-2xl text-white">
                <span className="font-semibold">{props.title}</span>
                <span className="whitespace-pre"> - </span>
                <span className="font-light text-lg">{props.company}</span>
            </h3>
